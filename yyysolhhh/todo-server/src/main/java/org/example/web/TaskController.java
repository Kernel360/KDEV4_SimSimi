...
public class TaskController {
    ...
    public ResponseEntity<Task> createTask(@RequestBody TaskRequest req) {
        var result = this.taskService.add(req.getTitle(), req.getDescription(), req.getDueDate()); //TODO var 왜 쓰는거지?
        return ResponseEntity.ok(result); //http 응답 body에 담아서 클라이언트에 반환
    }
    ...
    public ResponseEntity<List<Task>> getByStatus(@PathVariable TaskStatus status) { //@PathVariable - {status} 경로 변수를 status로 바인딩
    ...
    public ResponseEntity<Task> updateTask(@PathVariable Long id,
                                           @RequestBody TaskRequest task, BindingResult bindingResult) {
